...
// column first
glm::vec3 convertToColumnFirst(uint32_t resolution, uint32_t elementSize, uint32_t i)
{
	int index = i / elementSize;
	int x = index / resolution % resolution;
	int y = index % resolution;
	int z = index / resolution / resolution;
	glm::vec3 pos = glm::vec3((float)x / resolution, (float)y / resolution, (float)z / resolution);
	return pos;
}
...
{   
	...
	*/
	...
	const char* dir = "./generatedNoise"; // Path to the directory
	struct stat sb;
	if (stat(dir, &sb) == 0)
		std::cout << "The path   "  << dir << "   already exists." << std::endl;
	else
	{
		std::cout << "The path   " << dir << "   does exist." << std::endl;
	...
	int cloudBaseShapeTextureSize = 128;				// !!! If this is reduce, you hsould also reduce the number of frequency in the fmb noise  !!!
														// !!! If you use cloudBaseShapeTextureSize > 128 and save it as .tga, some softwares cannot open it, because a width over 128*128 is to large to open
	...
	parallel_for(int(0), int(cloudBaseShapeTextureSize), [&](int s) //for (int s = 0; s<gCloudBaseShapeTextureSize; s++)
	{
		...
		for (int t = 0; t<cloudBaseShapeTextureSize; t++)
		{
			for (int r = 0; r<cloudBaseShapeTextureSize; r++)
			{
				...
				// Perlin FBM noise
				const int octaveCount = 8;
				const float frequency = 4.0f;
				float perlinNoise = Tileable3dNoise::PerlinNoise(coord, frequency, octaveCount, 1.0, 0.5);

				// Worley 1
				const float worleyNoise0 = (1.0f - Tileable3dNoise::WorleyNoise(coord, 6));
				const float worleyNoise1 = (1.0f - Tileable3dNoise::WorleyNoise(coord, 12));
				const float worleyNoise2 = (1.0f - Tileable3dNoise::WorleyNoise(coord, 24));
				// Worley 2
				const float worleyNoise3 = (1.0f - Tileable3dNoise::WorleyNoise(coord, 10));
				const float worleyNoise4 = (1.0f - Tileable3dNoise::WorleyNoise(coord, 20));
				const float worleyNoise5 = (1.0f - Tileable3dNoise::WorleyNoise(coord, 40));
				// Worley 3
				const float worleyNoise6 = (1.0f - Tileable3dNoise::WorleyNoise(coord, 20));
				const float worleyNoise7 = (1.0f - Tileable3dNoise::WorleyNoise(coord, 40));
				const float worleyNoise8 = (1.0f - Tileable3dNoise::WorleyNoise(coord, 80));

				float PerlinWorleyNoise = 0.0f;
				{
					const float perlinWorleyNoise0 = (1.0f - Tileable3dNoise::WorleyNoise(coord, 6));
					const float perlinWorleyNoise1 = (1.0f - Tileable3dNoise::WorleyNoise(coord, 18));
					const float perlinWorleyNoise2 = (1.0f - Tileable3dNoise::WorleyNoise(coord, 30));

					float worleyFBM = perlinWorleyNoise0*0.625f + perlinWorleyNoise1*0.25f + perlinWorleyNoise2*0.125f;
					...
					//PerlinWorleyNoise = remap(perlinNoise, 0.0f, 1.0f, worleyFBM * 0.6, 1.0f); // "* 0.6" is to make it closer to the image displayed in their slides
					PerlinWorleyNoise = remap(perlinNoise, -worleyFBM, 1.0f, 0.0f, 1.0f); // this is an alternative way to make perlin-worley
					//if (PerlinWorleyNoise < 0) PerlinWorleyNoise = 0; // accompany with the alternative way, avoid minus value
					 
					//PerlinWorleyNoise = perlinNoise; // hold for check perlin noise generation
				}
				...
				float worleyFBM1 = worleyNoise3 * 0.625f + worleyNoise4 * 0.25f + worleyNoise5 * 0.125f;
				float worleyFBM2 = worleyNoise6 * 0.625f + worleyNoise7 * 0.25f + worleyNoise8 * 0.125f;
				...
				float value = 0.0;
				{
					...
					//value = remap(baseCloud, -(lowFreqFBM), 1.0f, 0.0f, 1.0f);
					// Saturate
	...
	); // end parallel_for
	{
		...
		// the original writeTGA function does not perform well, generates too bright results, so stbi is recommended
		...
		stbi_write_tga("./generatedNoise/perlinWorleyNoise.tga",       height, width, 4, cloudBaseShapeTexels);
		stbi_write_tga("./generatedNoise/perlinWorleyNoisePacked.tga", width, height, 4, cloudBaseShapeTexelsPacked);
		//stbi_write_bmp("perlworlnoise_stbi.bmp", width, height, 4, cloudBaseShapeTexels); // save into different format
	}
	...
	); // end parallel_for
	{
		...
		stbi_write_tga("./generatedNoise/worleyNoise.tga",       height, width, 4, cloudErosionTexels);
		stbi_write_tga("./generatedNoise/worleyNoisePacked.tga", width, height, 4, cloudErosionTexelsPacked);
	}
	...
	std::cout << std::endl;
	std::cout << "================  Noise generation is over  ================" << std::endl;
	std::cout << "Please check directory  ./generatedNoise  for the noises" << std::endl;
	std::cout << "Input any letter/number and press Enter to exit. (e.g. a + Enter)" << std::endl;
	std::getchar();
