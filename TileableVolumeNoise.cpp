...
float Tileable3dNoise::PerlinNoise(const glm::vec3& pIn, float frequency, int octaveCount, float amplitude, float attenuation, bool normalize)
{
	...
	float weight = amplitude; // Jing: changed this value
	...
	for (int oct = 0; oct < octaveCount; oct++)
	{
		...
		weight *= attenuation; // Jing: changed this line, it was weight *= weight (refer to https://github.com/Reputeless/PerlinNoise)
		frequency *= octaveFrenquencyFactor;
	}

	//float noise = (sum / weightSum) *0.5f + 0.5f;
	float noise = normalize? ((sum / weightSum) * 0.5f + 0.5f) : (sum * 0.5f + 0.5f); // Jing: this is not normalized version
