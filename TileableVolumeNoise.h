...
public:
	...
	/// @param p 3d coordinate in [0, 1], being the range of the repeatable pattern.
	static float PerlinNoise(const glm::vec3& p, float frequency, int octaveCount, float amplitude = 1.0f, float attenuation = 0.5f, bool normalize = false); // Jing: add several arguments for better control
